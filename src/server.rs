...
use pimalaya_email::{BackendBuilder, CompilerBuilder};
  ...
  thread::spawn(move || {
    ...
    loop {
      ...
      let main_message = match message {
        ...
        ServerCmd::GetAllEnvelopes(folder) => match backend.list_envelopes(&folder, 1024, 0) {
          Err(e) => to_main.send(ServerEvent::Error(e.into())),
          Ok(envelopes) => {
            let envelopes_as_map: HashMap<String, Envelope> = envelopes.to_vec().into_iter().map(|i| (i.id.clone(), i)).collect();
        ...
        ServerCmd::GetMessageBody(id, folder) => match backend.get_emails(&folder, vec![&id]) {
          Err(e) => to_main.send(ServerEvent::Error(e.into())),
          Ok(msg) => {
            let body = msg
              .first()
              ...
              .unwrap();
            to_main.send(ServerEvent::Body((id, String::from_utf8_lossy(&body).to_string())))
          },
        },
        ServerCmd::GetMboxes => match backend.list_folders() {
          Err(e) => to_main.send(ServerEvent::Error(e.into())),
          Ok(folders) => {
            let mboxes: Vec<MboxName> = folders.iter().filter(|f| f.name != "[Gmail]").map(|folder| folder.name.clone()).collect();
